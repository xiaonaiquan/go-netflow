...
func (p *Process) copy() *Process {
	stats := &trafficStatsEntry{}
	if p.TrafficStats != nil {
		stats.In = p.TrafficStats.In
		stats.Out = p.TrafficStats.Out
		stats.InRate = p.TrafficStats.InRate
		stats.OutRate = p.TrafficStats.OutRate
	}
...
func (pm *processController) GetByPID(pid string, sec int) *Process {
	pm.RLock()
	defer pm.RUnlock()
	...
	// Return an analysed copy to avoid leaking the mutable inner process.
	copied := po.copy()
	copied.analyseStats(sec)
	return copied
}
