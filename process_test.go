...
func TestProcessControllerGetByPID(t *testing.T) {
	pm := NewProcessController(context.Background())
	...
	got := pm.GetByPID("123", 5)
	assert.NotNil(t, got)
	assert.Equal(t, "123", got.Pid)
	assert.EqualValues(t, 120, got.TrafficStats.In)
	assert.EqualValues(t, 40, got.TrafficStats.Out)
	assert.NotEqual(t, pm.Get("123"), got)

	miss := pm.GetByPID("404", 5)
	assert.Nil(t, miss)
}
