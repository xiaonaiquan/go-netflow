...
type Interface interface {
	...
	// param recentSeconds, the average of the last few seconds' value.
	GetProcessByPID(pid string, recentSeconds int) (*Process, error)
}
...
func (nf *Netflow) GetProcessByPID(pid string, recentSeconds int) (*Process, error) {
	if recentSeconds > maxRingSize {
		return nil, errors.New("windows interval must <= 15")
	}
	...
	if len(pid) == 0 {
		return nil, errors.New("pid cannot be empty")
	}

	proc := nf.processHash.GetByPID(pid, recentSeconds)
	if proc == nil {
